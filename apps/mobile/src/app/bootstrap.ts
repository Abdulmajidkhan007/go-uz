 ...
 *
 * Backend selection (env-only, read via react-native-config from apps/mobile/.env):
 *   - If VROOM_FIREBASE_* vars are present → real Firebase backend.
 *   - Otherwise → the VROOM_API_MODE client (mock by default).
 *
...
const firebaseConfig = loadFirebaseConfig({
  VROOM_FIREBASE_API_KEY: Config.VROOM_FIREBASE_API_KEY,
  VROOM_FIREBASE_AUTH_DOMAIN: Config.VROOM_FIREBASE_AUTH_DOMAIN,
  VROOM_FIREBASE_PROJECT_ID: Config.VROOM_FIREBASE_PROJECT_ID,
  VROOM_FIREBASE_APP_ID: Config.VROOM_FIREBASE_APP_ID,
  VROOM_FIREBASE_STORAGE_BUCKET: Config.VROOM_FIREBASE_STORAGE_BUCKET,
  VROOM_FIREBASE_MESSAGING_SENDER_ID: Config.VROOM_FIREBASE_MESSAGING_SENDER_ID,
});
...
function buildClient(): ApiClient {
  ...
  const appConfig = loadConfig({
    VROOM_API_MODE: Config.VROOM_API_MODE ?? 'mock',
    VROOM_API_BASE_URL: Config.VROOM_API_BASE_URL,
  });
